...
const getStatusOptions = (itemType: string) => {
  switch (itemType) {
    case 'Book':
      return [
        { value: 'WISHLIST', label: '📚 Want to Read', color: '#94a3b8' },
        { value: 'IN_PROGRESS', label: '📖 Reading', color: '#10b981' },
        { value: 'COMPLETED', label: '✅ Read', color: '#059669' },
        { value: 'DROPPED', label: '⛔ Dropped', color: '#ef4444' },
      ];
    case 'Movie':
      return [
        { value: 'WISHLIST', label: '📋 Want to Watch', color: '#94a3b8' },
        { value: 'IN_PROGRESS', label: '🎬 Watching', color: '#3b82f6' },
        { value: 'COMPLETED', label: '✅ Watched', color: '#0ea5e9' },
        { value: 'DROPPED', label: '⛔ Dropped', color: '#ef4444' },
      ];
    case 'TV Series':
      return [
        { value: 'WISHLIST', label: '📋 Want to Watch', color: '#94a3b8' },
        { value: 'IN_PROGRESS', label: '📺 Watching', color: '#8b5cf6' },
        { value: 'COMPLETED', label: '✅ Watched', color: '#7c3aed' },
        { value: 'DROPPED', label: '⛔ Dropped', color: '#ef4444' },
      ];
    case 'Music':
      return [
        { value: 'WISHLIST', label: '📋 Want to Listen', color: '#94a3b8' },
        { value: 'IN_PROGRESS', label: '🎵 Listening', color: '#ec4899' },
        { value: 'COMPLETED', label: '✅ Listened', color: '#db2777' },
        { value: 'DROPPED', label: '⛔ Dropped', color: '#ef4444' },
      ];
    default:
      return [
        { value: 'WISHLIST', label: '📚 Wishlist', color: '#94a3b8' },
        { value: 'IN_PROGRESS', label: '▶️ In Progress', color: '#10b981' },
...
const StatusBadge = ({ 
  ...
  const STATUS_OPTIONS = getStatusOptions(itemType);
        ...
        <>
          ...
          <div className="status-dropdown">
            {STATUS_OPTIONS.map(option => (
              <button
                key={option.value}
                className={`status-option ${option.value === status ? 'active' : ''}`}
                style={{ 
                  borderLeft: `4px solid ${option.color}`,
                  backgroundColor: option.value === status ? '#f7fafc' : 'white'
                }}
                onClick={(e) => {
                  e.stopPropagation();
...
const MyLibrary = () => {
  ...
  const handleStatusChange = async (id: number, newStatus: string) => {
    ...
    try {
      console.log('Updating status for item:', id, 'New status:', newStatus);
      ...
      let updatedItem;
      if (item.type === 'Book') {
        updatedItem = await libraryService.updateBookStatus(id, newStatus);
      } else if (item.type === 'Movie') {
        updatedItem = await libraryService.updateMovieStatus(id, newStatus);
      }
        ...
        setItems(prevItems =>
          prevItems.map(i =>
            i.id === id ? { 
              ...
              currentPage: updatedItem.currentPage,
            } : i
          )
        );
      }
        ...
        {currentItems.map((item) => (
          ...
          >
            ...
            <div className="item-content">
              ...
              {/* Progress Info (for books) */}
              {item.type === 'Book' && item.currentPage && item.pageCount && (
                <div className="progress-info">
                  <span>📄 {item.currentPage} / {item.pageCount} pages</span>
                </div>
              )}
