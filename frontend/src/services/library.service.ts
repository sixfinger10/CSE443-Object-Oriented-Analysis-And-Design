...
export interface MusicRequest {
  ...
  rating?: number;
  favorite?: boolean;
  mbid?: string;  // ✅ Last.fm Unique ID desteği eklendi
}
...
/**
 * ✅ GÜNCELLEME: Helper - Auth headers oluştur (X-User-Id ile)
 */
...
class LibraryService {
  ...
  async createTVSeries(series: TVSeriesRequest) {
    try {
      console.log('📺 Creating TV series with data:', series);
  ...
  async createMusic(music: MusicRequest) {
    try {
      console.log('🎵 Creating music with data:', music);
  ...
  async updateTVSeries(id: number, series: TVSeriesRequest) {
    try {
      const response = await api.put(`/series/${id}`, series, {
        headers: getAuthHeaders()
      });
      console.log('SUCCESS: TV Series updated:', response.data);
      return response.data;
    } catch (error: any) {
      console.error('ERROR: Updating TV series:', error);
      throw new Error(error.response?.data?.message || 'Failed to update TV series');
    }
  }

  // ✅ YENİ: Music update metodu eklendi
  async updateMusic(id: number, music: MusicRequest) {
    try {
      const response = await api.put(`/music/${id}`, music, {
        headers: getAuthHeaders()
      });
      console.log('SUCCESS: Music updated:', response.data);
      return response.data;
    } catch (error: any) {
      console.error('ERROR: Updating music:', error);
      throw new Error(error.response?.data?.message || 'Failed to update music');
    }
  }

  // ✅ UPDATE STATUS METODLARI
  ...
  async deleteTVSeries(id: number) {
    try {
      ...
      console.log('SUCCESS: TV Series deleted');
      return response.data;
    } catch (error: any) {
      console.error('ERROR: Deleting TV series:', error);
      throw new Error(error.response?.data?.message || 'Failed to delete TV series');
    }
  }

  // ✅ YENİ: Music delete metodu eklendi
  async deleteMusic(id: number) {
    try {
      const response = await api.delete(`/music/${id}`, {
        headers: getAuthHeaders()
      });
      console.log('SUCCESS: Music deleted');
      return response.data;
    } catch (error: any) {
